package com.ominimie.auth.api_key.rest;
...
public class ApiKeyResource {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<ApiKeyDTO> getApiKey(@PathVariable(name = "id") final Long id) {
        return ResponseEntity.ok(apiKeyService.get(id));
    }

    @PostMapping
    @ApiResponse(responseCode = "201")
    public ResponseEntity<Long> createApiKey(@RequestBody @Valid final ApiKeyDTO apiKeyDTO) {
        final Long createdId = apiKeyService.create(apiKeyDTO);
        return new ResponseEntity<>(createdId, HttpStatus.CREATED);
    }

    @PutMapping("/{id}")
    public ResponseEntity<Long> updateApiKey(@PathVariable(name = "id") final Long id,
    ...
    public ResponseEntity<Void> deleteApiKey(@PathVariable(name = "id") final Long id) {
